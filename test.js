...
describe("Configuration", function() {
		...
		describe("Application using configuration", function() {
		it('shows the configuration we are running', function(done){
			...
			request
				.get('/')
					.expect(200)
				.expect(function (res) {
					res.text.should.equal("Running configuration: local");
				})
				.end(done);
		});
		});
